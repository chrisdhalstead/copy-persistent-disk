...
$script:pddestfs = "\\hzn-79-cs1-cdh.betavmweuc.com\PDData\UserData\$un"
...
#Get Remote Path
Write-Log -Message "Remote Path: $script:pddestfs"
...
if (!(Test-Path $script:pddestfs))  {
  Write-Log("$tab $pddestfs not found-creating folder")
  New-Item -ItemType Directory -Path $script:pddestfs
...
Foreach ($sfile in $pddiffs) 
    {
      ...
      $destfile = $sfile.path -replace [Regex]::Escape("$PDPath"),[Regex]::Escape("$pddestfs")
